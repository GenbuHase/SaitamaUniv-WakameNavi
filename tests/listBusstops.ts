...
function listBusstops (start: string): object {
  const result = [];

  for (const routeName in Routes) {
    const route = Routes[routeName as keyof typeof Routes];

    if (route.find(busstop => busstop.id === start)) {
      result.push(...route.filter(busstop => busstop.id !== start));
    }
  }

  return result.filter((busstop, i, _) => _.findIndex(busstop002 => busstop002.id === busstop.id) === i);
}

console.log(listBusstops("00021229"));